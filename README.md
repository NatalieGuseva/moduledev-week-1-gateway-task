...
| Команда | Назначение |
|---|---|
| `flow validate <map.json>` | Семантическая проверка карты без записи в БД: достижимость всех шагов, хотя бы один достижимый `end`, отсутствие циклов и тупиков, точное соответствие `required_policy` и outcome'ов зарегистрированному action |
| `flow publish <map.json>` | Публикация версии карты. Идемпотентна: повторная публикация с тем же содержимым — не ошибка, с другим содержимым той же версии — `manifest.conflict` |
